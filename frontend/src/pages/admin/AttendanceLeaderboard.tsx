...
export const AttendanceLeaderboard: React.FC = () => {
  ...
  const handleAdjustClick = (item: LeaderboardItem) => {
    ...
    setAdjustmentValue(String(item.attendanceAdjustment || 0));
  };
        ...
        <div style={{
          ...
          <div style={{ background: 'white', padding: '1.5rem', borderRadius: 'var(--radius-lg)', width: '100%', maxWidth: '320px' }}>
            ...
            <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', marginBottom: '1rem' }}>
              Set manual adjustment for <strong>{adjustingUser.name}</strong>.
            </p>
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, marginBottom: '0.5rem' }}>Additional Days (+/-)</label>
              <input 
                type="number" 
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              <button onClick={() => setAdjustingUser(null)} className="btn-secondary" style={{ flex: 1 }}>Cancel</button>
              <button onClick={handleAdjustSave} className="btn-primary" style={{ flex: 1 }}>Save</button>
            </div>
          </div>
        </div>
      )}
